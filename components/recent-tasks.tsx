...
export function RecentTasks({ onTaskChanged }: { onTaskChanged?: () => void }) {
  ...
  return (
    <div className="bg-gradient-to-br from-purple-700 via-purple-500 to-pink-400 rounded-2xl p-6 shadow-md">
      <h2 className="text-xl font-bold mb-4 text-white drop-shadow-[0_1px_4px_rgba(60,0,80,0.7)]">Recent Tasks</h2>
      <div className="space-y-3">
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => (
            <div key={task.id} className="flex items-center justify-between py-3 px-4 rounded-lg border border-gray-300 bg-white/80 hover:bg-white transition-all cursor-pointer">
              ...
              <div className="flex flex-col items-end ml-4">
                <span className={`text-xs font-bold ${task.deadline ? (getDaysLeft(task.deadline) === "Overdue" ? "text-gray-900" : "text-white") : "text-gray-400"}`} style={{ textShadow: task.deadline ? (!task.deadline ? '0 1px 4px rgba(60,0,80,0.5)' : undefined) : undefined }}>{task.deadline ? getDaysLeft(task.deadline) : "No deadline"}</span>
              </div>
            </div>
          ))
        )}
